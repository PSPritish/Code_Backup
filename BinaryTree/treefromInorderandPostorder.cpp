...
Node *bt(unordered_map<int, int> &mp, vector<int> &postorder, int postStart, int postEnd, int inStart, int inEnd)
{
    ...
    int brkpt = mp[postorder[postEnd]];
    Node *root = new Node(postorder[postEnd]);

    root->left = bt(mp, postorder, postStart, postStart + (brkpt - inStart) - 1, inStart, brkpt - 1);
    root->right = bt(mp, postorder, postStart + (brkpt - inStart), postEnd - 1, brkpt + 1, inEnd);
